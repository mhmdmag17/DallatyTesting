...
class ApiController extends Controller
{
    ...
    public function getProjectDetail(Request $request)
    {
        ...
        try {
            ...
            $project = Projects::with('customer:id,name,profile,email,mobile,address')
                ->with('gallary_images')
                ->with('documents')
                ->with('plans')
    ...
    public function getProjects(Request $request){
        try{
            ...
            $projectsQuery = Projects::where('status',1)
                        ->with('category:id,slug_id,image,category', 'gallary_images', 'customer:id,name,profile,email,mobile')
                        ->select('id', 'slug_id', 'city', 'state', 'country', 'title', 'type', 'image', 'status', 'location','category_id','added_by');
