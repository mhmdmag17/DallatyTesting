...
class _PropertyMapScreenState extends State<PropertyMapScreen> {
  ...
  Future<void> loadDefaultCity() async {
    ...
    final latLng = await getCityLatLong(HiveUtils.getCityPlaceId());
    //Animate camera to location
    await (await completer.future).animateCamera(
  ...
  //   LabelMarker(
  //     icon: selectedMarker == i
  //         ? customIconSelected ?? BitmapDescriptor.defaultMarker
  //         : element.propertyType == '0'
  //             ? customIconSell ?? BitmapDescriptor.defaultMarker
  //             : customIconRent ?? BitmapDescriptor.defaultMarker,
  //     label: Constant.currencySymbol +
  //         element.price.priceFormate(context: context),
  //     markerId: MarkerId('$i'),
  //     onTap: () async {
  //       selectedMarker = i;
  //       propertyId = element.propertyId;
  ...
  //       setState(() {});
  //       await fetchProperty(element.propertyId);
  //     },
  //     position: LatLng(
  //       double.parse(element.latitude),
  //       double.parse(element.longitude),
  ...
  //         ? Colors.red
  //         : (element.propertyType == '0' ? Colors.green : Colors.orange),
  //   ),
  // )
  //     .then(
  //   (value) {
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const Text('Sell').color(context.color.inverseSurface),
              ],
            ),
          ),
          const SizedBox(
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const Text('Rent').color(context.color.inverseSurface),
              ],
            ),
          ),
        ],
...
//       {double width = 80, double height = 80}) async {
...
//       )
//       ..layout(maxWidth: width * 2); // Allow text to wrap if needed
//
//     // Calculate total height needed
//     final totalHeight = height + painter.height + 20; // 20 for padding
//
//     // Create a canvas to draw on
//     final pictureRecorder = ui.PictureRecorder();
//     final canvas = Canvas(pictureRecorder)
//       ..drawImage(fi.image, Offset.zero, Paint());
//
//     // Draw a semi-transparent background for the text
//     final bgPaint = Paint()..color = Colors.black.withOpacity(0.5);
//     canvas.drawRect(
//       Rect.fromLTWH(0, height, width, totalHeight - height),
//       bgPaint,
//     );
//
//     // Draw the text
//     painter.paint(
//       canvas,
//       Offset((width - painter.width) / 2, height + 10), // 10 for top padding
//     );
//
